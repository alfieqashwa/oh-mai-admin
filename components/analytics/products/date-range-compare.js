          ...
          <Popover.Button className="flex items-center justify-between px-4 py-2 mt-2 space-x-8 bg-N200 bg-opacity-20">
            <div className="text-left">
              ...
              endPrevious?.[0] ? (
                <>
                  <h4
                    className={`w250 hover:text-G400 transition duration-300 ease-in-out ${
                      open ? 'text-P700' : ''
                    }`}
                  >
                    current period ({format(startCurrent?.[0], 'MMM d')} -{' '}
                    {format(endCurrent?.[0], 'MMM d, yyyy')})
                  </h4>
                  <p className="normal-case w400">
                    vs. Previous Period {format(startPrevious?.[0], 'MMM d')} -{' '}
                    {format(endPrevious?.[0], 'MMM d, yyyy')}
                  </p>
                </>
              ) : (
                <div className="pr-80">
            ...
            <Popover.Panel className="absolute z-20 w-screen max-w-sm mt-3 transform -translate-x-1/2 left-1/2 sm:px-0 lg:max-w-lg">
              <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
                <div className="relative pt-4 bg-N100">
                  <h4 className="text-center uppercase font-secondary w250 text-N800">
                    select a date range
                  </h4>
                  <RadioGroup
                    as="div"
                    value={plan}
                    onChange={setPlan}
                    className="flex flex-row items-center justify-center mt-4 mb-6"
                  >
                    <RadioGroup.Option value="presets">
                      {({ active, checked }) => (
                        <button
                          type="button"
                          className={`px-16 py-3 border rounded-none rounded-l  ${
                            checked ? 'bg-P700' : 'bg-N100'
                          }`}
                        >
                          <h4
                            className={`w250 font-secondary ${
                              checked ? 'text-N100' : 'text-P700'
                            }`}
                          >
                            presets
                          </h4>
                    ...
                    <RadioGroup.Option value="custom">
                      {({ active, checked }) => (
                        <button
                          type="button"
                          className={`px-16 py-3 border rounded-none rounded-r  ${
                            checked ? 'bg-P700' : 'bg-N100'
                          }`}
                        >
                          <h4
