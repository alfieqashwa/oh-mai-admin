    ...
    <form onSubmit={onSubmit} className="text-center">
      ...
      <div className="px-4 pb-6 mt-4 -mx-6 bg-N100">
        ...
        <div className="flex items-start justify-between mt-2">
          <p className="pt-5 pl-8 text-black W400 whitespace-nowrap">
            Same range in
          </p>
          <Listbox
            as="div"
          ...
          >
            {({ open }) => (
              <>
                <Listbox.Button className="bg-N200 mt-1.5 h-12 w-full">
                  <div className="flex items-center justify-between px-4">
                    <p
                      className={`${
                        open ? 'text-P700' : 'text-N450'
                      } normal-case w400 hover:text-P700`}
                    >
                      {compareTo.name}
                    </p>
                    <ChevronDownIcon
                ...
                >
                  <Listbox.Options
                    static
                    className="w-full mt-1.5 text-left shadow-md bg-N100"
                  >
                    {prevYearList.map((y) => (
                      <Listbox.Option
                        key={y.id}
        ...
        <div className="flex items-center justify-center pt-8 space-x-4">
          <button
            type="button"
            onClick={onReset}
            className="py-3 uppercase border px-14 bg-N50 border-N300"
          >
            <h4 className="text-N450 w250">reset</h4>
          </button>
          <button
            type="submit"
            disabled={compareTo.id === 0}
...
const initialState = [{ id: 0, name: 'Select a year', disabled: true }]
