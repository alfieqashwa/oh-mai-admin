...
export default function KOL() {
  ...
  const { data: prod, error: prodErr } = useSWR(kols);
    ...
    if (prod) {
      ...
      (async () => {
        for (var i = 0; i < prod.kols.length; i++) {
          ...
          let a = await client.request(KOL_USER_INFO, { id: kol.user_id });
          console.log(a);
  ...
  async function deleteKolMutation(id) {
    await client.request(DELETE_KOL, { id: id });
  }
  async function deleteKolsMutation(id) {
    for (var i = 0; i < id.length; i++) {
      await client.request(DELETE_KOL, { id: id[i] });
    }
  }
